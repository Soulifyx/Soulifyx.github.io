...
d3.csv("principals-in-schools-age.csv").then(function(dataset){
	...
	var groupedDatas = d3.nest()
			...
			.rollup(function(v) {
				...
				v.forEach(function(d){
					d2[d.level_of_school] = d.no_of_principals;
				});
				d2.Total = d3.sum(v, function(d) {return +d.no_of_principals;});
				return d2;
			})
			.entries(dataset)
	    ...
	    var stackBar = barParent.selectAll("barParent")
	    	...
	    	.on("mouseover", function(d){
	    		...
	    		if(d.data.Sex == "MF")
	    			...
	    			gender = "female";
	    		tooltip.select(".count").html(((d[0] - d[1]) * -1) + " " + gender + " principals" );
